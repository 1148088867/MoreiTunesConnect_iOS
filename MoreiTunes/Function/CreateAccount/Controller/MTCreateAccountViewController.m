...
psx(MTCreateAccountView, createAccountView);
...
- (void)setUI {
    MTCreateAccountView *createAccountView = [[MTCreateAccountView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
    ...
    if (self.accountModel) {
        MTAccountModel *model = self.accountModel;
...
- (void)saveDidClick {
    ...
    if (hasText) {
        ...
        weakOBJ(account);
        [account setObject:[NSDate dateCurrentTimeWithFormat:@"yyyy-MM-dd HH:mm:ss"] forKey:@"adMTime"];
        [keys enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            QMUITextField *field = [weak_self.createAccountView viewWithTag:idx+100];
            [weak_account setObject:[field.text stringByReplacingOccurrencesOfString:@" " withString:@""].encryptAESString forKey:obj];
        }];
        [MTSQL.store putObject:account withId:account[@"adMTime"] intoTable:MTAccountTableName];
        if (self.accountModel) {
            NSLog(@"%@", self.accountModel.adMTime);
            [MTSQL.store deleteObjectById:self.accountModel.adMTime fromTable:MTAccountTableName];
        }
        if (self.createAccountSuccess) {
            self.createAccountSuccess([MTAccountModel yy_modelWithJSON:MTSQL.keyValueItems.lastObject.itemObject]);
